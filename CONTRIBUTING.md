...
Перед кожним злиттям змін у парсері обов'язково збільшуйте номер версії у `build.gradle.kts`:

`version = 25` → `version = 26`

CloudStream кешує версії джерел, тому без збільшення версії автоматичне оновлення не спрацює у користувачів.
Нова версія повинна бути на одиницю більшою за поточну версію в репозиторії.
